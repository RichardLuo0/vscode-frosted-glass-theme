...
1. 运行 `frosted-glass-theme.setup` 如果你想要启用mica。
1. 如果在上一步中没有适用于你主题的主题修改器, 运行 `frosted-glass-theme.generateThemeMod` 然后把结果加到 `workbench.colorCustomizations`。
1. 重启vscode。
...
## 自定义
* 运行 `frosted-glass-theme.openConfig` 可以查看当前的全部配置。本扩展的所有设置都会被合并到这个配置文件中。
...
* `Fake mica` 默认关闭。开启 `frosted-glass-theme.fakeMica.enabled` 设置以开启此功能。另外因为本扩展不会自动给你的主题添加透明度，你需要同时改变主题颜色，以下是一个示例:
    ...
    你也可以试试 `frosted-glass-theme.generateThemeMod` 指令以自动生成一个主题修改器，然后把结果加入 `workbench.colorCustomizations`。
...
* `frosted-glass-theme.animation` 的key定义在 `src-inject/animation.ts` 的 `selectorMap` 中，也可以是css选择器。值可以是定义在 `src-inject/vscode-frosted-glass-theme.css` 的 `Animation` 中的css动画或者定义在 `src-inject/effect/effect.ts` 的 `effectMap` 中的effect。然而，effect不会被css timing function影响，他们只是被 `animationstart` 事件触发。
